/****************************************************************************
** Meta object code from reading C++ file 'updatethread.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.7.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../RSStalk/updatethread.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'updatethread.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.7.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_UpdateThread_t {
    QByteArrayData data[6];
    char stringdata0[64];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_UpdateThread_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_UpdateThread_t qt_meta_stringdata_UpdateThread = {
    {
QT_MOC_LITERAL(0, 0, 12), // "UpdateThread"
QT_MOC_LITERAL(1, 13, 13), // "alreadyUpdate"
QT_MOC_LITERAL(2, 27, 0), // ""
QT_MOC_LITERAL(3, 28, 11), // "updateError"
QT_MOC_LITERAL(4, 40, 16), // "showUpdateDialog"
QT_MOC_LITERAL(5, 57, 6) // "noFeed"

    },
    "UpdateThread\0alreadyUpdate\0\0updateError\0"
    "showUpdateDialog\0noFeed"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_UpdateThread[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       4,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    0,   34,    2, 0x06 /* Public */,
       3,    1,   35,    2, 0x06 /* Public */,
       4,    1,   38,    2, 0x06 /* Public */,
       5,    0,   41,    2, 0x06 /* Public */,

 // signals: parameters
    QMetaType::Void,
    QMetaType::Void, QMetaType::Int,    2,
    QMetaType::Void, QMetaType::QString,    2,
    QMetaType::Void,

       0        // eod
};

void UpdateThread::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        UpdateThread *_t = static_cast<UpdateThread *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->alreadyUpdate(); break;
        case 1: _t->updateError((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 2: _t->showUpdateDialog((*reinterpret_cast< QString(*)>(_a[1]))); break;
        case 3: _t->noFeed(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (UpdateThread::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&UpdateThread::alreadyUpdate)) {
                *result = 0;
                return;
            }
        }
        {
            typedef void (UpdateThread::*_t)(int );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&UpdateThread::updateError)) {
                *result = 1;
                return;
            }
        }
        {
            typedef void (UpdateThread::*_t)(QString );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&UpdateThread::showUpdateDialog)) {
                *result = 2;
                return;
            }
        }
        {
            typedef void (UpdateThread::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&UpdateThread::noFeed)) {
                *result = 3;
                return;
            }
        }
    }
}

const QMetaObject UpdateThread::staticMetaObject = {
    { &QThread::staticMetaObject, qt_meta_stringdata_UpdateThread.data,
      qt_meta_data_UpdateThread,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *UpdateThread::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *UpdateThread::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_UpdateThread.stringdata0))
        return static_cast<void*>(const_cast< UpdateThread*>(this));
    return QThread::qt_metacast(_clname);
}

int UpdateThread::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QThread::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 4)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 4;
    }
    return _id;
}

// SIGNAL 0
void UpdateThread::alreadyUpdate()
{
    QMetaObject::activate(this, &staticMetaObject, 0, Q_NULLPTR);
}

// SIGNAL 1
void UpdateThread::updateError(int _t1)
{
    void *_a[] = { Q_NULLPTR, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}

// SIGNAL 2
void UpdateThread::showUpdateDialog(QString _t1)
{
    void *_a[] = { Q_NULLPTR, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 2, _a);
}

// SIGNAL 3
void UpdateThread::noFeed()
{
    QMetaObject::activate(this, &staticMetaObject, 3, Q_NULLPTR);
}
QT_END_MOC_NAMESPACE
